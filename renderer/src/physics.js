...
export async function updatePhysics(dt, now) {
    ...
    if (!isIdle) {
      if (fish.personality === CONFIG.PERSONALITIES.CALMO) {
        ...
        if (dist < 40 && Math.random() < 0.08) {
          ...
          if (skin.loaded && skin.animations.bite) {
            skin.currentAnim = "bite";
            skin._animIndex = 0;
          }
        }
      }
      ...
      const maxSpeed =
        speedBase +
        wobble +
        (fish.personality === "caotico" ? 100 : 20) +
        (foods.length > 0 ? 150 : 0);
      const s = Math.hypot(fish.vx, fish.vy);
      if (s > maxSpeed) {
        fish.vx = (fish.vx / s) * maxSpeed;
        fish.vy = (fish.vy / s) * maxSpeed;
      }

      const targetRotation = Math.max(
        -0.6,
        Math.min(0.6, fish.vy * 0.003 * fish.dir),
      );
      ...
      if (!isIdle && globals.activeWindowRect) {
        ...
        if (
          fish.x + halfW > aw.left &&
          fish.x - halfW < aw.right &&
          fish.y + halfH > aw.top &&
          fish.y - halfH < aw.bottom
        ) {
          ...
          if (aw.bottom + halfH <= canvas.height - 20)
            validEscapes.push({
              dist: distBottom,
              side: "bottom",
              target: aw.bottom + halfH,
            });
          if (aw.left - halfW >= 20)
            validEscapes.push({
              dist: distLeft,
              side: "left",
              target: aw.left - halfW,
            });
          if (aw.right + halfW <= canvas.width - 20)
            validEscapes.push({
              dist: distRight,
              side: "right",
              target: aw.right + halfW,
            });
      ...
      if (skin.loaded) {
        ...
        if (skin.currentAnim === "bite") {
          if ((skin._animIndex || 0) >= anim.length - 1 && Math.random() < 0.5)
            skin.currentAnim = "swim";
        }
        advanceAnimation(dt);
      }
    }
  ...
  toys.forEach((t) => {
    ...
    if (t.x > canvas.width - t.r) {
      t.x = canvas.width - t.r;
      t.vx *= -t.bounciness;
    }
